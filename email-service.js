...
async function sendEmailViaEmailJS(email, name, title, message) {
    try {
...
async function sendRegistrationConfirmationEmail(user) {
    ...
    const message = `Benvenuto/a su SbobinaMente! La tua registrazione è stata completata con successo. Email: ${user.email}. Il tuo account è ora attivo.`;
    
    return sendEmailViaEmailJS(user.email, user.nome, title, message);
}
...
async function sendPaymentConfirmationEmail(order, digitalsAccess) {
    ...
    const message = `Grazie per il tuo acquisto!\n\nOrdine: ${order.id}\nTotale: ${order.total.toFixed(2)}€\n\nArticoli:\n${itemsList}\n\nRiceverai presto i dettagli di spedizione.`;
    
    return sendEmailViaEmailJS(order.deliveryInfo.email, order.deliveryInfo.fullName, title, message);
}
...
async function sendPDFDownloadEmail(customerEmail, customerName, pdfAccess) {
    ...
    return sendEmailViaEmailJS(customerEmail, customerName, title, message);
}

// 4. EMAIL GENERICA DI ASSISTENZA
async function sendSupportEmail(senderEmail, senderName, subject, message) {
    return sendEmailViaEmailJS(senderEmail, senderName, `Assistenza: ${subject}`, message);
}
